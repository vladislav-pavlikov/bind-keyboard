...
class BindKeyboard {
  ...
  #bindings: Record<Types.EventType, Map<Types.KeyСombination, Classes.KeybindEntry>>
  ...
  constructor (props: Types.ConstructorProps = {}) {
    ...
    if (props?.initialBindings) {
      for (const { keyCombination, callback, preventRepeat = true, type = 'keypress' } of props.initialBindings) {
   ...
   */
  #debugLog = ({ ev, keyCombination, callback }: { ev: Event, keyCombination: Types.KeyСombination, callback?: EventListener }): void => {
    if (this.#debug === 2) {
      console.log(`%c${ev.type} %c${keyCombination} %c${callback || ''}`, 'color: red', '', 'color: green')
    }
  ...
  #listener = (ev: Event): void => {
    ...
    const entry = this.#bindings?.[ev.type as Types.EventType]?.get(keyCombination)

    if ((ev as KeyboardEvent).repeat && entry?.preventRepeat) {
      return
    }

    if (this.#debug) this.#debugLog({ ev, keyCombination, callback: entry?.callback })
   ...
   */
  getKeybind = (keyCombination: Types.KeyСombination | Types.KeyСombinationConstruct, type: Types.EventType = 'keypress'): Classes.KeybindEntry | undefined =>
   ...
   */
  getAllBindings = (): Classes.KeybindEntry[] => [...this.#bindings.keydown, ...this.#bindings.keypress, ...this.#bindings.keyup].map(el => el[1])
  ...
  add = (
    ...
    const entry = new Classes.KeybindEntry(helpers.keyParser(keyCombination), callback, type, preventRepeat)
   ...
   * @throws {Classes.KeybindError} When an invalid EventType is provided.
   */
  remove = (keyCombination: Types.KeyСombination | Types.KeyСombinationConstruct, type: Types.EventType = 'keypress'): boolean | undefined => {
    if (!['keypress', 'keydown', 'keyup'].includes(type)) {
      throw new Classes.KeybindError('Wrong EventType.')
    }
   ...
   */
  removeAll = (): void => { Object.values(this.#bindings).forEach((map) => { map.clear() }) }
  ...
  startListners = (): void => {
    Object
      .keys(this.#bindings)
      .forEach((key) => { this.#target.addEventListener(key, this.#listener) })
  }
  ...
  stopListners = (): void => {
    Object
      .keys(this.#bindings)
      .forEach((key) => { this.#target.removeEventListener(key, this.#listener) })
  }
